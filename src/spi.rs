...
impl<const POLARITY: SckPolarity, const PHASE: SckPhase> PayloadSPIBitBang<POLARITY, PHASE>{
    #[inline(always)]
    fn receive_bit(result: &mut u32, recv_pin: &impl InputPin<Error=void::Void>) {
        *result = (*result << 1) | (recv_pin.is_high().unwrap() as u32);
    }
    #[inline(always)]
    fn send_bit(bit: u8, send_pin: &mut impl OutputPin) {
        if bit > 0 {send_pin.set_high().ok();}
        else {send_pin.set_low().ok();}
    }
    ...
    fn receive_after_second_edge(&mut self, len: u8, cs_pin: &mut impl OutputPin) -> u32 {
        ...
        while current_pos < len {
            ...
            delay_cycles(80); // duty cycle correction
            self.sck.toggle().ok();
            Self::receive_bit(&mut result, &self.miso);
            current_pos += 1;
        }
        cs_pin.set_high().ok();
        result
    ...
    fn send_before_second_edge(&mut self, len: u8, data: u32, cs_pin: &mut impl OutputPin) {
        ...
        while current_pos < len {
            self.sck.toggle().ok();
            delay_cycles(80); // duty cycle correction
            let bit: u8 = (data & (1_u32 << (len - current_pos - 1_u8))) as u8;
            Self::send_bit(bit, &mut self.mosi);
            self.sck.toggle().ok();
            current_pos += 1;
        }
        cs_pin.set_high().ok();
...
// Transformation function to convert between subtypes
