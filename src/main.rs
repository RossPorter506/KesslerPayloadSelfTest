...
fn main() -> !{
    if let Some(periph) = msp430fr2355::Peripherals::take() {
        ...
        // Display measured current from Aperture Test PCB when set in Current Source mode
        // In this mode, a hard set 1kohm resistor should set the current source to ~750uA
        // Expect to read ~750 uA before closing the TVAC door
        // Expect to read ~0 uA when DUT is in vacuum and heater is off.
        let measured_aperture_adc_mv = payload.aperture_test_adc.read_voltage_from(&pcb_mapping::sensor_locations::APERTURE_TEST_CURRENT_SENSOR, &mut payload_spi_controller);
        ...
        uwriteln!(serial_writer, "Enabling Heater").ok();
        let mut payload = payload.into_enabled_heater();
        uwriteln!(serial_writer, "Performing aperture current sensing test").ok();
