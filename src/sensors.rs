use crate::{adc::{TemperatureSensor, ADC, TetherSensor, MiscSensor}, dac::{DAC, DACCommand}, digipot::Digipot, spi::PayloadSPI}; 
...
pub struct SensorReader {
    pub tether_adc: ADC<P6, Pin2, TetherSensor>, //TODO: Rejig ADC so we can just put TetherADC here, etc
    pub temperature_adc: ADC<P6, Pin0, TemperatureSensor>,
    pub misc_adc: ADC<P5, Pin4, MiscSensor>,
    pub dac: DAC,
    pub digipot: Digipot,
}
impl SensorReader{
    //Temperature sensors
    pub fn get_lms_temperature_kelvin(&mut self, temp_sensor: &TemperatureSensor, spi_bus: &mut dyn PayloadSPI) -> u16{
        let adc_voltage = self.temperature_adc.read_voltage_from(temp_sensor, spi_bus);
        lms_temperature_eq(adc_voltage)
    }
    pub fn get_payload_temperature_kelvin(&mut self, temp_sensor: &TemperatureSensor, spi_bus: &mut dyn PayloadSPI) -> u16{
        let adc_voltage = self.temperature_adc.read_voltage_from(temp_sensor, spi_bus);
    ...
    /* Supplies */
    // Heater
    pub fn set_heater_voltage(&mut self, mut target_voltage_millivolts: u16, spi_bus: &mut dyn PayloadSPI){
    ...
    pub fn get_heater_voltage_millivolts(&mut self, spi_bus: &mut dyn PayloadSPI) -> u16{
    ...
    pub fn get_heater_current_milliamps(&mut self, spi_bus: &mut dyn PayloadSPI) -> i16{
    ...
    //Tether Bias
    pub fn set_tether_bias_voltage(&mut self, mut target_voltage_millivolts: u32, spi_bus: &mut dyn PayloadSPI){
    ...
    pub fn get_tether_bias_voltage_millivolts(&mut self, spi_bus: &mut dyn PayloadSPI) -> i32 {
        let adc_voltage = self.tether_adc.read_voltage_from(&TETHER_BIAS_VOLTAGE_SENSOR, spi_bus);
        tether_bias_voltage_eq(adc_voltage)
    }
    pub fn get_tether_bias_current_milliamps(&mut self, spi_bus: &mut dyn PayloadSPI) -> i32 {
        let adc_voltage = self.tether_adc.read_voltage_from(&TETHER_BIAS_CURRENT_SENSOR, spi_bus);
        tether_bias_current_eq(adc_voltage)
    }

    //Cathode Offset
    pub fn set_cathode_offset_voltage(&mut self, mut target_voltage_millivolts: u32, spi_bus: &mut dyn PayloadSPI){
    ...
    pub fn get_cathode_offset_voltage_millivolts(&mut self, spi_bus: &mut dyn PayloadSPI) -> i32 {
    ...
    pub fn get_cathode_offset_current_milliamps(&mut self, spi_bus: &mut dyn PayloadSPI) -> i32 {
    ...
    //Repeller
    pub fn get_repeller_voltage_millivolts(&mut self, spi_bus: &mut dyn PayloadSPI) -> i32 {
