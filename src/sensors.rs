...
use crate::{adc::{TemperatureSensor, TetherADC, MiscADC, TemperatureADC}, dac::{DAC, DACCommand}, digipot::Digipot, spi::{PayloadSPI, SckIdleHigh, SckIdleLow}}; 
...
impl PayloadController{
    ...
    //Temperature sensors
    pub fn get_lms_temperature_kelvin(&mut self, temp_sensor: &TemperatureSensor, spi_bus: &mut impl PayloadSPI<SckIdleHigh>) -> u16{
        let adc_voltage = self.temperature_adc.read_voltage_from(temp_sensor, spi_bus);
        lms_temperature_eq(adc_voltage)
    }
    pub fn get_payload_temperature_kelvin(&mut self, temp_sensor: &TemperatureSensor, spi_bus: &mut impl PayloadSPI<SckIdleHigh>) -> u16{
        let adc_voltage = self.temperature_adc.read_voltage_from(temp_sensor, spi_bus);
    ...
    /* Supplies */
    // Heater
    pub fn set_heater_voltage(&mut self, mut target_voltage_millivolts: u16, spi_bus: &mut impl PayloadSPI<SckIdleLow>){
    ...
    pub fn get_heater_voltage_millivolts(&mut self, spi_bus: &mut impl PayloadSPI<SckIdleHigh>) -> u16{
        let adc_voltage_millivolts = self.tether_adc.read_voltage_from(&HEATER_VOLTAGE_SENSOR, spi_bus);
        heater_voltage_eq(adc_voltage_millivolts)
    }
    pub fn get_heater_current_milliamps(&mut self, spi_bus: &mut impl PayloadSPI<SckIdleHigh>) -> i16{
        let adc_voltage_millivolts = self.tether_adc.read_voltage_from(&HEATER_CURRENT_SENSOR, spi_bus);
        heater_current_eq(adc_voltage_millivolts)
    }

    //Tether Bias
    pub fn set_tether_bias_voltage(&mut self, mut target_voltage_millivolts: u32, spi_bus: &mut impl PayloadSPI<SckIdleLow>){
    ...
    pub fn get_tether_bias_voltage_millivolts(&mut self, spi_bus: &mut impl PayloadSPI<SckIdleHigh>) -> i32 {
        let adc_voltage = self.tether_adc.read_voltage_from(&TETHER_BIAS_VOLTAGE_SENSOR, spi_bus);
        tether_bias_voltage_eq(adc_voltage)
    }
    pub fn get_tether_bias_current_milliamps(&mut self, spi_bus: &mut impl PayloadSPI<SckIdleHigh>) -> i32 {
        let adc_voltage = self.tether_adc.read_voltage_from(&TETHER_BIAS_CURRENT_SENSOR, spi_bus);
        tether_bias_current_eq(adc_voltage)
    }

    //Cathode Offset
    pub fn set_cathode_offset_voltage(&mut self, mut target_voltage_millivolts: u32, spi_bus: &mut impl PayloadSPI<SckIdleLow>){
    ...
    pub fn get_cathode_offset_voltage_millivolts(&mut self, spi_bus: &mut impl PayloadSPI<SckIdleHigh>) -> i32 {
    ...
    pub fn get_cathode_offset_current_milliamps(&mut self, spi_bus: &mut impl PayloadSPI<SckIdleHigh>) -> i32 {
    ...
    //Repeller
    pub fn get_repeller_voltage_millivolts(&mut self, spi_bus: &mut impl PayloadSPI<SckIdleHigh>) -> i32 {
